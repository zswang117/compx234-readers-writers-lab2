...
class ReadersWritersMonitor:
    ...
    def end_read(self, reader_id: int) -> None:
        ...
        with self.condition:
            # TODO: Replace 'pass' with your logic
            self.active_readers -= 1
            print(f"Reader {reader_id} has stopped reading. Active readers: {self.active_readers}")
        ...
        with self.condition:
            # TODO: Replace 'pass' with your logic
             while self.active_readers > 0 or self.active_writers > 0:
                print(f"Writer {writer_id} waiting to write...")
                self.condition.wait()
            self.active_writers = 1
            print(f"Writer {writer_id} has started writing. Active writers: {self.active_writers}")
    ...
    def end_write(self, writer_id: int) -> None:
        ...
        with self.condition:
            # TODO: Replace 'pass' with your logic
             self.active_writers = 0
            print(f"Writer {writer_id} has stopped writing. Active writers: {self.active_writers}")
            self.condition.notify_all()
